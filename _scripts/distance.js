var fitbitAccessToken;
    ...
    var processDistance = function(timeSeries) {
        return timeSeries['activities-distance'].map(
            function(measurement){
                ...
                return [month + "-" + day, Number.parseFloat(measurement.value) ]; //converts min count to integer
            } //end measure func 
        ); //end map
    } // end processSteps


    var graphDistance = function(timeSeries) {
        
        stepData = new google.visualization.DataTable;
        stepData.addColumn('string', 'Date');
        stepData.addColumn('number', 'Kilometers');
    ...
    var graphGoal = function(timeSeries) {
        ...
        var goalData = google.visualization.arrayToDataTable([ 
            ['Progress','Percent'],
            ['Distance Completed', currentTotal],
            ['Distance To Go', goal]
            ]);
...
window.addEventListener('load', function(){
    fetch(
        'https://api.fitbit.com/1/user/-/activities/distance/date/2016-08-22/1w.json',
        {
            headers: new Headers({
